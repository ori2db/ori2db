...
=======
	...
	public byte[] httpUrlConnection(String path, String method, String content, Map<String, String> requestProperty, int readTimeout, int connTimeout) {
		...
		try {
			...
			/* 检查请求方式 */
			if (StringUtils.isEmpty(method) || (!NetworkConstants.GET.equals(method) && !method.equals(NetworkConstants.POST))) {
				method = NetworkConstants.GET;
			}
			conn.setRequestMethod(method);
			conn.setDoInput(true);
			...
			/* 如果是post请求，并且请求全文不是空，写入数据 */
			if (NetworkConstants.POST.equals(method) && !StringUtils.isEmpty(content)) {
				OutputStream out = conn.getOutputStream();
				out.write(content.getBytes());
				out.flush();
				out.close();
